...
scenarioFr['resourceDemand'] =  pd.concat(
    (
        pd.DataFrame(data = {
              ...
              'electricity': np.array(elec_demand.loc[(slice(None),'electricity'),'areaConsumption']*interpolate(anualElec,y_ref(year,y_act))/anualElec[2020]), # incrising demand of electricity (hypothesis : ADEME)
              'hydrogen': interpolate(hourlyH2,y_ref(year,y_act))*np.ones(nHours), # base-load consumption of H2 (hypothesis : ADEME)
              'gaz': np.zeros(nHours),
...
for k, year in enumerate(yearList[:-1]):
    ...
    scenarioFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                ...
                'minInstallCapacity':interpolate(min_install_capacity,y_ref(year,y_act)),'maxInstallCapacity': interpolate(max_install_capacity,y_ref(year,y_act)),
                'EmissionCO2': 0, 'Conversion': {'electricity': 1},
                'EnergyNbhourCap': 0, # used for hydroelectricity
                'minCumulCapacity': interpolate(min_cumul_capacity,y_ref(year,y_act)),'maxCumulCapacity': interpolate(max_cumul_capacity,y_ref(year,y_act))}
            }
         )
    )
    ...
    scenarioFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                ...
                'EnergyNbhourCap': 0, # used for hydroelectricity
                'minCumulCapacity': interpolate(min_cumul_capacity,y_ref(year,y_act)),'maxCumulCapacity': interpolate(max_cumul_capacity,y_ref(year,y_act)),'RampConstraintPlus':0.04,'RampConstraintMoins':0.04}
            }
         )
    )
    ...
    scenarioFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                ...
                'EnergyNbhourCap': 0, # used for hydroelectricity
                'minCumulCapacity': interpolate(min_cumul_capacity,y_ref(year,y_act)),'maxCumulCapacity': interpolate(max_cumul_capacity,y_ref(year,y_act)),'RampConstraintPlus':0.06,'RampConstraintMoins':0.06}
            }
         )
    )
    ...
    scenarioFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                ...
                'minCumulCapacity': 0,'maxCumulCapacity': interpolate(max_cumul_capacity,y_ref(year,y_act))}
            }
         )
    )
...
biogasPrice = {2020:140,2030: 120,2040: 100,2050: 80}
...
scenarioFr['resourceImportCO2eq'] = pd.concat(
    (
        pd.DataFrame(data={
            ...
            'gaz': max(0, 0.03 * (1 - (y_ref(year,y_act) - yearZero)/(2050 - yearZero))) * 29 / 13.1 + 203.5  * (1 - tech_eco_data.get_biogas_share_in_network_RTE(y_ref(year,y_act))), # Taking 100 yr GWP of methane and 3% losses due to upstream leaks. Losses drop to zero in 2050.
            'gazNat': max(0, 0.03 * (1 - (y_ref(year,y_act) - yearZero)/(2050 - yearZero))) * 29 / 13.1 + 203.5  * (1 - tech_eco_data.get_biogas_share_in_network_RTE(y_ref(year,y_act))), # Taking 100 yr GWP of methane and 3% losses due to upstream leaks. Losses drop to zero in 2050.

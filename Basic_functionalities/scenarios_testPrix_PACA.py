...
scenarioPACAtest['resourceDemand'] =  pd.concat(
    (
        pd.DataFrame(data = { 
              'YEAR': year, 
...
scenarioPACAtest['conversionTechs'] = pd.concat(scenarioPACAtest['conversionTechs'], axis=1)
...
scenarioPACAtest['storageTechs'] =  pd.concat(scenarioPACAtest['storageTechs'], axis=1)

scenarioPACAtest['carbonTax'] = pd.DataFrame(data=np.linspace(0.0675,0.165, nYears),
    index=yearList, columns=('carbonTax',))

scenarioPACAtest['carbonGoals'] = pd.DataFrame(data=np.linspace(974e6, 205e6, nYears),
    index=yearList, columns=('carbonGoals',))

impBiogasCap=np.linspace(0, 5e6, nYears)
# impH2Cap=np.linspace(0, 30e6, nYears)
scenarioPACAtest['maxImportCap'] = pd.concat(
    (
...
scenarioPACAtest['maxExportCap'] = pd.concat(
    (
        pd.DataFrame(index=[year],data={
            'electricity': 0,#10e6,
            'gazNat': 0,
...
scenarioPACAtest['gridConnection'] = pd.read_csv(inputPath+'CalendrierHTB_TIME.csv', sep=',', decimal='.', skiprows=0,
                                comment="#").set_index(["TIMESTAMP"])

scenarioPACAtest['economicParameters'] = pd.DataFrame({
    'discountRate':[0.04], 
    'financeRate': [0.04]
    }
)
...
scenarioPACAtest['resourceImportPrices'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioPACAtest['resourceImportCO2eq'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioPACAtest['convTechList'] = ["WindOnShore", "WindOffShore_flot", "Solar", 'SMR','SMR + CCS1','SMR + CCS2','CCS1','CCS2','electrolysis_PEMEL','electrolysis_AEL' ,"curtailment",'SMR_elec','SMR_elecCCS1']
ctechs = scenarioPACAtest['convTechList']
...
scenarioPACAtest['availability'] = availabilityFactor.loc[(slice(None), slice(None), itechs)]

scenarioPACAtest["yearList"] = yearList
scenarioPACAtest["transitionFactors"] =pd.DataFrame(
    {'TECHNO1':['SMR','SMR'],

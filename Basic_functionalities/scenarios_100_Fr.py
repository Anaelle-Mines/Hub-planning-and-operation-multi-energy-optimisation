...
anualElec={2020:492e6,2030:554.2e6,2040:669.1e6,2050:723.3e6}
hourlyH2={2020:[0],2030:[1825],2040:[2400],2050:[3710]}
scenarioFr100['resourceDemand'] =  pd.concat(
    (
        pd.DataFrame(data = {
              'YEAR': year,
...
for k, year in enumerate(yearList[:-1]):
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity= [8000, 33200 , 57400, 74000]
    min_cumul_capacity = [8000, 33200 , 57400, 74000]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='low', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech: 
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity=[4000,35100,130300,208000]
    min_cumul_capacity =[4000,35100,130300,208000]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='low', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity= [63100,57600,35000,0]
    min_cumul_capacity = [63100,57600,35000,0]
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity=[10000,5000,10000,20000]
    min_cumul_capacity = [0,0,0,0]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech:
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity= [5000,50000,50000,500]
    min_cumul_capacity =[5000,50000,50000,500]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity= [5000,5000,10000,20000]
    min_cumul_capacity =[0,0,0,0]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech:
    ...
    max_cumul_capacity= [0,6500,10000,21000]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioFr100['conversionTechs'].append(
        pd.DataFrame(data={tech:
...
scenarioFr100['conversionTechs'] =  pd.concat(scenarioFr100['conversionTechs'], axis=1)

scenarioFr100['storageTechs'] = []
for k, year in enumerate(yearList[:-1]):
    tech = "Battery"
    max_install_capacity = [0,5000,129000,256000]
    max_install_power=[0,500,12900,25600]
    ...
    scenarioFr100['storageTechs'].append(
        pd.DataFrame(data={tech: 
    ...
    tech = "saltCavernH2_G"
    max_install_capacity = [1000000,1000000,1000000,1000000]
    max_install_power=[100000,100000,100000,100000]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioFr100['storageTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year,
...
scenarioFr100['storageTechs'] =  pd.concat(scenarioFr100['storageTechs'], axis=1)

scenarioFr100['carbonTax'] = pd.DataFrame(data=np.linspace(0.0675,0.165, nYears),
    index=yearList, columns=('carbonTax',))

scenarioFr100['carbonGoals'] = pd.DataFrame(data=np.linspace(974e6, 205e6, nYears),
    index=yearList, columns=('carbonGoals',))

scenarioFr100['maxBiogasCap'] = pd.DataFrame(data=np.linspace(0, 310e6, nYears),
    index=yearList, columns=('maxBiogasCap',))

scenarioFr100['gridConnection'] = pd.read_csv(inputPath+'CalendrierHPHC_TIME.csv', sep=',', decimal='.', skiprows=0,
                                comment="#").set_index(["TIMESTAMP"])

scenarioFr100['economicParameters'] = pd.DataFrame({
    'discountRate':[0.04], 
    'financeRate': [0.04]
    }
)
...
scenarioFr100['resourceImportPrices'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioFr100['resourceImportCO2eq'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioFr100['convTechList'] = ["WindOnShore", "WindOffShore", "Solar", "CCG", "TAC","Coal_p", "OldNuke","NewNuke","IntercoIn","IntercoOut","curtailment","HydroReservoir","HydroRiver","CCG_H2","TAC_H2","electrolysis_AEL"]
ctechs = scenarioFr100['convTechList']
...
scenarioFr100['availability'] = availabilityFactor.loc[(slice(None), slice(None), itechs)]

scenarioFr100["yearList"] = yearList
scenarioFr100["transitionFactors"] =pd.DataFrame(
    {'TECHNO1':[],
    'TECHNO2':[],

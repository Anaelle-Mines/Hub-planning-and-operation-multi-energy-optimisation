...
InputFolder='Data/Input_chaleur/'
OutputFolder='Data/Output_chaleur/'
...
TechParameters=pd.DataFrame([['Wind',0,130000,45000,30,0,0,10000],['Solar',0,800000,15000,20,0,0,10000],['PAC',0,600000,36000,20,0,0,10000]],columns=['TECHNOLOGIES','powerCost','investCost','operationCost','lifeSpan','EmissionCO2','minCapacity','maxCapacity']).set_index('TECHNOLOGIES')
StorageParameters=pd.DataFrame([['Battery','electricity',223600,239940,4472,15,100,1000],['Heat_storage','heat',10000,8000,650,20,100,1000]],columns=['STOCK_TECHNO','resource','storagePowerInvestCost','storageEnergyInvestCost','storageOperationCost','storagelifeSpan','p_max','c_max']).set_index('STOCK_TECHNO')
ConversionFactors=pd.DataFrame([['electricity','Wind',1],['electricity','Solar',1],['electricity','PAC',-0.33],['heat','Wind',0],['heat','Solar',0],['heat','PAC',1]],columns=['RESOURCES','TECHNOLOGIES','conversionFactor']).set_index(['RESOURCES','TECHNOLOGIES'])
StorageFactors=pd.DataFrame([['electricity','Battery',0.92,0.0085,1.09],['electricity','Heat_storage',0,0,0],['heat','Battery',0,0,0],['heat','Heat_storage',1,0.004,1]],columns=['RESOURCES','STOCK_TECHNO','storageFactorIn','dissipation','storageFactorOut']).set_index(['RESOURCES','STOCK_TECHNO'])

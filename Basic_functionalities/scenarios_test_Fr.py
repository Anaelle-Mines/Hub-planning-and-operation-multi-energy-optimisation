...
scenarioTestFr['resourceDemand'] =  pd.concat(
    (
        pd.DataFrame(data = { 
              'YEAR': year, 
...
scenarioTestFr['conversionTechs'] = []
for k, year in enumerate(yearList[:-1]):
    ...
    scenarioTestFr['conversionTechs'].append(
    ...
    min_cumul_capacity = [15000,0,0,0]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioTestFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    min_cumul_capacity = [11000,0,0,0]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioTestFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    min_cumul_capacity = [63100,0,0,0]
    capex, opex, lifespan = tech_eco_data.get_capex_new_tech_RTE(tech, hyp='ref', year=year)
    scenarioTestFr['conversionTechs'].append(
        pd.DataFrame(data={tech:
                { 'YEAR': year, 'Category': 'Electricity production',
    ...
    max_install_capacity = [100000,100000,100000,100000]
    min_install_capacity=[0,0,0,0]
    max_cumul_capacity= [5000,5000,5000,5000]
...
scenarioTestFr['conversionTechs'] =  pd.concat(scenarioTestFr['conversionTechs'], axis=1)

scenarioTestFr['storageTechs'] = []
for k, year in enumerate(yearList[:-1]):
    tech = "Battery"
    ...
    scenarioTestFr['storageTechs'].append(
        pd.DataFrame(data={tech: 
...
scenarioTestFr['storageTechs'] =  pd.concat(scenarioTestFr['storageTechs'], axis=1)

scenarioTestFr['carbonTax'] = pd.DataFrame(data=np.linspace(0.0675,0.165, nYears),
    index=yearList, columns=('carbonTax',))

scenarioTestFr['carbonGoals'] = pd.DataFrame(data=np.linspace(974e6, 205e6, nYears),
    index=yearList, columns=('carbonGoals',))

scenarioTestFr['maxBiogasCap'] = pd.DataFrame(data=np.linspace(0, 310e6, nYears),
    index=yearList, columns=('maxBiogasCap',))

scenarioTestFr['gridConnection'] = pd.read_csv(inputPath+'CalendrierHPHC_TIME.csv', sep=',', decimal='.', skiprows=0,
                                comment="#").set_index(["TIMESTAMP"])

scenarioTestFr['economicParameters'] = pd.DataFrame({
    'discountRate':[0.04], 
    'financeRate': [0.04]
    }
)
...
scenarioTestFr['resourceImportPrices'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioTestFr['resourceImportCO2eq'] = pd.concat(
    (
        pd.DataFrame(data={
            'YEAR': year, 
            'TIMESTAMP': t, 
...
scenarioTestFr['convTechList'] = ["WindOnShore", "WindOffShore", "Solar", "CCG", "TAC","Coal_p", "OldNuke","NewNuke","IntercoIn","IntercoOut","curtailment","HydroReservoir","HydroRiver"]
ctechs = scenarioTestFr['convTechList']
...
scenarioTestFr['availability'] = availabilityFactor.loc[(slice(None), slice(None), itechs)]

scenarioTestFr["yearList"] = yearList
scenarioTestFr["transitionFactors"] =pd.DataFrame(
    {'TECHNO1':[],
    'TECHNO2':[],

...
def systemModel_MultiResource_WithStorage(scenario, isAbstract=False):
    ...
    inputDict["maxBiogasCap"] = inputDict["maxBiogasCap"]
    ...
    model.carbon_goal = Param(model, default=0,
                              initialize=carbonGoals['carbonGoals'].squeeze().to_dict(), domain=NonNegativeReals)
    model.carbon_taxe = Param(model, default=0, initialize=CarbonTax['carbonTax'].squeeze().to_dict(),
    ...
    for COLNAME in StorageParameters:
        if COLNAME not in ["STOCK_TECHNO", "AREAS", "YEAR"]:  ### each column in StorageParameters will be a parameter
            exec("model." + COLNAME + " =Param(model.STOCKTECHNO,domain=Any,default=0," +
    ...
    model.carbonCosts_Pvar = Var(model, domain=NonNegativeReals)
    ...
    def importCostsDef_rule(model, y, res):
        return sum((model.importCost[t, res] * model.importation_Dvar[t, res]) for t in model.TIMESTAMP) == model.importCosts_Pvar[y, res]
    ...
    def CarbonCosts_rule(model):
        return model.carbonCosts_Pvar == sum(model.carbon_Pvar[t] * model.carbon_taxe for t in model.TIMESTAMP)
    model.CarbonCostsCtr = Constraint(model, rule=CarbonCosts_rule)
    ...
    def TurpeCtr_rule(model,res):
        if res == 'electricity':
            return model.turpeCosts_Pvar[res] == (
                        sum(model.HTA[t] * model.importation_Dvar[t, res] for t in TIMESTAMP) + model.max_PS_Dvar[
                   'P'] * 16310 + (model.max_PS_Dvar['HPH'] - model.max_PS_Dvar[ 'P']) * 15760 + (

...
def systemModel_MultiResource_WithStorage(scenario, zone,isAbstract=False):
    ...
    model.pipeCost = Param(model.YEAR_invest, mutable=False,initialize=pipeCost.loc[:, 'pipeCost'].squeeze().to_dict())
    ...
    def importCostsDef_rule(model, y, res):
        return sum(
            (model.importCost[y, t, res] * f3(r, y) * (model.importation_Dvar[y, t, res]-model.exportation_Dvar[y,t,res])) for t in model.TIMESTAMP) == \
    ...
    def turpePPA_rule(model,y,t,res):
        if res == 'electricity':
            return model.turpeQuantity[y,t,res] == model.importation_Dvar[y,t,res] + model.power_Dvar[y,t,'WindOnShorePPA']+ model.power_Dvar[y,t,'WindOffShorePPA']+ model.power_Dvar[y,t,'SolarPPA']
        else :
    ...
    def PuissanceSouscrite_rule(model, y, t, res):
        if res == 'electricity':
            if t in TIMESTAMP_P:
                return model.max_PS_Dvar[y, 'P'] >= model.turpeQuantity[y, t, res] + model.exportation_Dvar[y,t,res]  # en MW
            elif t in TIMESTAMP_HPH:
                return model.max_PS_Dvar[y, 'HPH'] >= model.turpeQuantity[y, t, res] + model.exportation_Dvar[y,t,res]
            elif t in TIMESTAMP_HCH:
                return model.max_PS_Dvar[y, 'HCH'] >= model.turpeQuantity[y, t, res] + model.exportation_Dvar[y,t,res]
            elif t in TIMESTAMP_HPE:
                return model.max_PS_Dvar[y, 'HPE'] >= model.turpeQuantity[y, t, res] + model.exportation_Dvar[y,t,res]
            elif t in TIMESTAMP_HCE:
                return model.max_PS_Dvar[y, 'HCE'] >= model.turpeQuantity[y, t, res] + model.exportation_Dvar[y,t,res]
        else:
            return Constraint.Skip

...
def GetElectricPriceModel(elecProd, marketPrice,ResParameters,TechParameters,capaCosts, carbonContent,conversionFactor,isAbstract=False):
    """
    ...
    model.YEAR_TIMESTAMP_TECNOLOGIES = model.YEAR * model.TIMESTAMP * model.TECHNOLOGIES
    ...
    model.LastCalled = Param(model.YEAR_TIMESTAMP, default=0,
                              initialize=marketPrice.loc[:, "LastCalled"].squeeze().to_dict(), domain=Any)
    model.elecProd = Param(model.YEAR_TIMESTAMP_TECNOLOGIES, default=0,
    ...
    model.carbon_taxe = Param(model.YEAR, default=0,initialize={2:0.1,3:0.13,4:0.15}, domain=NonNegativeReals)
    ...
    def TotalCostsDef_rule(model,y,tech):
        return sum(sum(model.elecProd[y,t,tech]*(-model.conversionFactor[res,tech]*model.importCosts[y,t,res]) for res in ['uranium','gaz','hydrogen',]) + model.elecProd[y,t,tech]*(model.varCosts[y-1,tech])+ model.elecProd[y,t,tech]*model.carbonContent[y,t]*model.carbon_taxe[y] for t in model.TIMESTAMP) + model.capaCosts[y,tech] == model.TotalCosts[y,tech]
    ...
    def meritDef_rule(model,y,tech):
        ...
        if tech == 'OldNuke' :
            return sum((-model.conversionFactor[res, 'OldNuke']) * model.importCosts[y, 1, res] for res in ['uranium','gaz','hydrogen',]) + model.varCosts[y-1,'OldNuke'] + model.AjustFac[y,'OldNuke'] <= sum((-model.conversionFactor[res, 'CCG']) * model.importCosts[y, 1, res] for res in ['uranium','gaz','hydrogen',]) + model.varCosts[y-1,'CCG'] + model.AjustFac[y,'CCG']
        if tech == 'CCG' :
            return sum((-model.conversionFactor[res, 'CCG']) * model.importCosts[y, 1, res] for res in ['uranium','gaz','hydrogen',]) + model.varCosts[y-1,'CCG'] + model.AjustFac[y,'CCG'] <= sum((-model.conversionFactor[res, 'TAC']) * model.importCosts[y, 1, res] for res in ['uranium','gaz','hydrogen',]) + model.varCosts[y-1,'TAC'] + model.AjustFac[y,'TAC']
        else :

...
def PlotCapacityAndImportationCosts(Variables) :
    ...
    df = pd.DataFrame([list(Variables['capacityCosts']['capacityCosts'] / 1000000)],
                      columns=list(Variables['capacityCosts']['TECHNOLOGIES']))
    ...
    df = pd.DataFrame([list(Variables['importCosts']['importCosts'] / 1000000)],
                      columns=list(Variables['importCosts']['RESSOURCES']))
...
def PlotCarbonCosts(Variables):
    ...
    df = pd.DataFrame([list(Variables['carbonCosts']['carbonCosts'] / 1000000)],
...
def PlotH2Production(Variables,year='2013') :
    ...
    fig7 = fig7.update_layout(title_text="Production H2 (en MW)",
                              xaxis_title="heures de l'année")
    plotly.offline.plot(fig7, filename='Power.html')
    return
